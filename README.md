...
从 Releases 下载 `kimi-session-manager.exe`，**双击即用**——不需要安装 Node.js 或任何依赖，会自动打开浏览器。
...
- 收藏星标和别名保存在 `~/.kimi-session-manager/session-meta.json`；交接文档保存在 `~/.kimi-session-manager/handoff/`
